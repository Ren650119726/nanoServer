local JoinRoomUI = class("JoinRoomUI", cc.load("mvc").ViewBase)
...
JoinRoomUI.RESOURCE_FILENAME = "layout/join_room.csb"
JoinRoomUI.RESOURCE_BINDING = {
...
function JoinRoomUI:inputNum(num)
    ...
    if #self.inputs == 6 then
        ...
        local deskId = table.concat(self.inputs)
        UIStack.popUI()
        WaitUI.show()
...
function JoinRoomUI:onReset()
    self.gameAssistant.playBtnClickedSound()
    self:reset()
    self.inputs = {}
end

function JoinRoomUI:onDelete()
...
function JoinRoomUI:onClose()
    self.gameAssistant.playCloseUISound()
    UIStack.popUI()
end

function JoinRoomUI:onNum(sender)
    self.gameAssistant.playBtnClickedSound()
    self:inputNum(sender:getTitleText())
end

function JoinRoomUI:onCreate(gameAssistant)
    self:reset()
    self.inputs = {}
