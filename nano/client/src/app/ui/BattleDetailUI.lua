...
local BattleDetailUI = class("BattleDetailUI", cc.load("mvc").ViewBase)
local UIStack = require "packages.mvc.UIStack"
local gameAssistant = require "app.logic.gameAssistant"
local BattleDetailItemUI = class("BattleItem", cc.load("mvc").ViewBase)
...
BattleDetailUI.RESOURCE_FILENAME = "layout/history_detail.csb"
BattleDetailUI.RESOURCE_BINDING = {
...
BattleDetailItemUI.RESOURCE_FILENAME = "layout/history_detail_item.csb"
BattleDetailItemUI.RESOURCE_BINDING = {
    nContainer = { id = "container" },
    tId = { id = "id" },
    tTime = { id = "time" },
    tScore0 = { id = "score0" },
...
function BattleDetailItemUI:onCreate(showBg, item)
    self.item = item
...
function BattleDetailItemUI:getSize()
    return self.size
end

function BattleDetailItemUI:onPlayback(sender)
    WaitUI.show("正在获取数据")
...
------------------------------------------
function BattleDetailUI:onCloseWindow(sender)
    gameAssistant.playCloseUISound()
    UIStack.popUI()
end

function BattleDetailUI:init()
end

function BattleDetailUI:addInfo(item)
    ...
    local item = BattleDetailItemUI:create("", "", #self.historyDetail % 2 == 0, item)
