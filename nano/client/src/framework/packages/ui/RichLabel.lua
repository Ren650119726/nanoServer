...
local labelparser = require 'gamecore.core.labelparser'
...
--	end
    if 'div' == label then
        return RichLabel.parserLabel
    elseif 'img' == label then
        return RichLabel.parserImg
    end
	return parser
end
...
-- <div> Parser
--
function RichLabel:parserLabel(params, default)
	-- 将字符串拆分成一个个字符
	local content = params.content
	if content then
		--params.charlist = {params.content}--self:stringToChars(content)
	...
	for index, char in pairs(charlist) do
		...
		if label then
            ...
            label:setAdditionalKerning(self._default.charSpace)
		end
	end
	return labellist
end

function RichLabel:parserImg(params, default)
